...
def main():
    ...
    def prepare_features(examples):
        ...
        if sent2_cname is not None and sent3_cname is None:
            for idx in range(total):
        ...
        # If hard negative exists
        if sent2_cname is not None and sent3_cname is not None :
            for idx in range(total):
                if examples[sent2_cname][idx] is None:
                    examples[sent2_cname][idx] = " "
        ...
        if sent3_cname is not None:
            for key in sent_features:
                features[key] = [
                    [sent_features[key][i + total * j] for j in range(sent_num)] for i
                    in range(total)]
